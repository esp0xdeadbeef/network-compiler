...
- FS-030-HDS-010-SDS-020-SMS-010 selector-fanout policy-bypass classification corrected | state=solved | evidence=tests/FS-030-HDS-010-SDS-020-SMS-010.sh | notes=shared upstream-selector multi-core fanout and shared downstream-selector multi-access fanout are valid topology and do not create traffic-path authority without modeled relations
- FS-620-HDS-010-SDS-010-SMS-010 compiler isolation classification construction proof restored | state=solved | evidence=tests/FS-620-HDS-010-SDS-010-SMS-010.sh
...
- implemented-fix: Normalize the address-free named DNS service, provider core, dual-stack requester binding, provider path, and egress choice into `site.dns`; merge only warning-free recursive services and relations into the compiler communication contract. Core-hosted services now retain provider-node ownership through compiled services and traffic paths. Fatal warning profiles emit no DNS path authority; an explicitly static provider-bound upstream retains only the selected relationship while emitting its permitted warning.
- evidence: `NETWORK_REPO_DIRECT_TEST_OK=1 NETWORK_LABS_PATH=/home/deadbeef/github/network-labs bash tests/FS-525-HDS-010-SDS-010-SMS-010.sh` proves the positive multi-egress profile plus missing, literal, invalid, ambiguous, and static-upstream warnings. Candidate identities are sorted, input permutation is byte-equivalent apart from site identity, and warning records contain no address values.
