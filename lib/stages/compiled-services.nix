...
let
  ...
  normalizeServicePolicy =
    ...
    let
      _required = policyFields.validateRequired siteKey serviceName policy;

      _noInference = policyFields.validateNoInference siteKey serviceName policy;

      # FS-590-HDS-010-SDS-010-SMS-040: discovery authority inference denial with specific diagnostics
      requireDiscoveryAuthorityInferenceDenial = import ./discovery-authority-inference-denial.nix {
        inherit lib;
      };

      _discoveryAuthorityInferenceDenial = builtins.deepSeq [
      ...
      requireDiscoveryBoundaryDecision = import ./discovery-boundary-decision.nix { inherit lib; };

      _discoveryBoundaryDecision = builtins.deepSeq [
      ...
      requireNoUnderlayAuthority = import ./underlay-authority.nix { inherit lib; };

      _underlayAuthorityBoundary = builtins.deepSeq [
        (requireNoUnderlayAuthority siteKey serviceName policy)
      ] true;

      requireManagementBoundary = import ./compiled-services-management-boundary.nix { inherit lib; };

      _managementBoundary = builtins.deepSeq [
        (requireManagementBoundary siteKey serviceName policy)
      ] true;

      # FS-630-HDS-010-SDS-010-SMS-040: denied-path preservation boundary
      requireNonEmptyDeniedPaths = import ./denied-path-boundary.nix { inherit lib; };

      _deniedPathBoundary = builtins.deepSeq [
      ...
      requireMediaDeniedPathPreservation = import ./media-denied-path-preservation.nix { inherit lib; };

      _mediaDeniedPathPreservationBoundary = builtins.deepSeq [
      ...
      requireMediaPayloadAuth = import ./media-payload-authorization.nix { inherit lib; };

      _mediaPayloadAuthBoundary = builtins.deepSeq [
      ...
      requirePrinterPayloadAuth = import ./printer-payload-authorization.nix { inherit lib; };

      _printerPayloadAuthBoundary = builtins.deepSeq [
...
map (
  ...
  // lib.optionalAttrs (svc ? providerNode) {
    ...
    addressAuthority = svc.addressAuthority or "model-allocated-service-prefix";
    recursionMode = svc.recursionMode or "iterative";
  }
